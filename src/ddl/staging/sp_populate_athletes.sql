CREATE PROCEDURE IF NOT EXISTS fetch_athletes()
LANGUAGE SQL
AS 
$$
DECLARE
    ...
    fetch_athletes_exception EXCEPTION (-20001, 'Exception raised in fetch_athletes()');
BEGIN
    -- Step 1: Get the last page
    SET (last_page, api_response_code) = (
        SELECT PARSE_JSON(json):last_page::NUMBER AS last_page,
        SELECT PARSE_JSON(json):_metadata:timestamp::NUMBER AS api_response_code
        FROM TABLE(get_json('https://api.triathlon.org/v1/athletes?category_id=42&per_page=1000', '0201b661afadf43392e4c7dcaed533fe'))
    );

    -- Step 2: Generate pages and fetch athlete data
    RETURN (
        WITH pages AS (
            SELECT
