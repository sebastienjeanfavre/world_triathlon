...
-- CREATE OR REPLACE TABLE staging.athletes AS
WITH pages AS (
    SELECT 
...
SELECT
    f.value:athlete_id::NUMBER(16,0) AS athlete_id,
    ...
    f.value:athlete_yob::NUMBER(4,0) AS athlete_yob,
    f.value:athlete_country_id::NUMBER(16,0) AS athlete_country_id,
...
select * from staging.athletes where ATHLETE_FIRST = 'Loanne'; -- 104222
select count(*) from staging.athletes;